...
```bash
$ docker run -it --rm -e SERVERIP='172.217.9.46' --cap-add=NET_ADMIN -p 8080:80 ghcr.io/soarinferret/iptablesproxy
```

Optionally, you can define `SERVERIP` and `HOSTIP` to specify a specific TCP port mapping.

```bash
$ docker run -it --rm -e SERVERIP='172.217.9.46' -e SERVERPORT='3200' -e HOSTPORT='80' --cap-add=NET_ADMIN -p 8080:80 ghcr.io/soarinferret/iptablesproxy
```
